...
void ThreeViewWidget::setupRenderersAndController()
{
	...
	connect(m_controller, &ThreeViewController::imageDragUpdated,
		this, &ThreeViewWidget::imageDragUpdated);

	connect(m_controller, &ThreeViewController::imageDragReset,
		this, &ThreeViewWidget::imageDragReset);
}
...
void ThreeViewWidget::setModeToMeasurement(InteractionMode mode, bool state)
{
	if (!state) {
